$(function () {
    $("#indexBackground").attr("style","margin-top: -1px;background: url('/img/b"+Math.round(Math.random()*(8-1)+1)+".png')  center center / cover no-repeat;")
        ...
        success:function (data) {
            ...
            $.each(data.list, function (index, item) {
                $("<div class=\"ui attached animate__animated animate__fadeInUp\"></div>").append(
                    $("<div class=\"d-web-info-box\" data-wow-duration=\"0.4s\"></div>").on("click", function () {
                        ...
                        $("<div class=\"ui mobile stackable grid\"></div>").append(
                            ...
                            $("<div class=\"seven wide column\" style='overflow: hidden;'></div>").append(
                                $("<img  class=\"ui rounded image centered Itemimg\"/>")
                                    .attr("src", item.firstPicture).on("click", function () {
                                    window.location.href = "/toBlog/" + item.id;
                                })
                            )
                        ).append(
    ...
    function setTime() {
    /*此处为网站的创建时间*/
    var create_time = Math.round(new Date(Date.UTC(2021, 12, 13, 12, 54, 10)).getTime() / 1000);
